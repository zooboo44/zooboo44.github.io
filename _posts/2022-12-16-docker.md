...
categories: Services
---
### Since accidentally deleting the VM with docker containers, this is not an accurate representation of the current state of my homelab. Only used for refrence purposes for the moment. Will update when its set back up. Probably going to use kubernetes anyway so this will not be accruate
